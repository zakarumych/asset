...
#[cfg_attr(
    ...
    serde(
        bound(
            serialize = "I: Serialize + Eq + Hash", deserialize = "I: DeserializeOwned + Eq + Hash"
        )
    )
)]
#[derive(Derivative)]
#[derivative(
    Clone,
    Debug(bound = "I: Eq + Hash + Debug"),
    PartialEq(bound = "I: Eq + Hash"),
    Eq(bound = "I: Eq + Hash")
)]
                        ...
                        if *repeat > laps {
                            *repeat -= laps;
                            ...
                            match animation.next {
                                None => (0, Current::Still(animation.range.end - 1)),
                                Some((ref id, ref repeat)) => (
                                    left,
                                    Current::Animated(Animation {
                                        elapsed: 0,
